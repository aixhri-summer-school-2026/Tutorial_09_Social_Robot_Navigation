...
class SocialMPPINode(object):
    def __init__(self):
        ...
        self.costmap = None
        self.cost_map_width = None
        self.cost_map_height = None
        self.pose = None
        self.twist = None
        self.cmd_vel = None
    ...
    def get_out_of_bounds_mask(self, x, y):
        """
        Helper function to check if the grid coordinates are out of bounds
        """
        out_of_bounds = (x < 0) | (x >= self.cost_map_width) | (y < 0) | (y >= self.cost_map_height)
    ...
    def get_cell_cost(self, x, y):
        ...
        # Flat index for the costmap data
        flat_indices = y0 * self.cost_map_width + x0
        costs = cost_data[flat_indices].astype(np.unint8)  # Ensure costs are in uint8 format
        return costs
    ...
    def computeVelocityCommands(self, costmap, pose, twist):
        ...
        self.cost_map_width = costmap.info.width
        self.cost_map_height = costmap.info.height
